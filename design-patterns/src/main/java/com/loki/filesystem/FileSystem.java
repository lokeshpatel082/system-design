...
public class FileSystem {
    ...
    public List<String> ls(String path) throws DirectoryNotFound {
        ...
        for (int i = 1; i < pathList.length; i++) {
            ...
            if (!startFile.files.containsKey(dir)) {
                throw new DirectoryNotFound("Can not found directory \"" + path);
            }
            startFile = startFile.files.get(dir);
        }
        if (!startFile.isDir) {
            throw new DirectoryNotFound("given path" + path + " is not a directory");
        }
    ...
    public void rm(String path) {
        ...
        if (parentFile == null) {
            throw new InvalidPath("can not remove directory" + path);
        }
    ...
    public void cd(String path) {
        String newPath = (path.charAt(0) == '/') ? path : currentFile + "/" + path;
        if (!isValidPath(path)) {
            throw new InvalidPath("Not a valid path");
        }
    ...
    public void addContentToFilePath(String filePath, String content) {
